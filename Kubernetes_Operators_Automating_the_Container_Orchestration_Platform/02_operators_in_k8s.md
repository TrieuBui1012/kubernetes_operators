...
## 2. Tài nguyên Tùy chỉnh (Custom Resources - CRs)
...
## 3. Bộ điều khiển tùy chỉnh (Custom Controllers)
...
## 4. Phạm vi hoạt động của Operator (Operator Scopes)
- **Phạm vi Namespace (Namespace Scope):** Đây là phạm vi được khuyến nghị trong hầu hết các trường hợp. Việc giới hạn Operator hoạt động trong một namespace duy nhất mang lại tính linh hoạt cao, cho phép các nhóm chạy hoặc nâng cấp các phiên bản Operator/ứng dụng một cách độc lập mà không ảnh hưởng tới toàn bộ hệ thống.
- **Phạm vi Cụm (Cluster-Scoped Operators):** Một số Operator đặc thù cần quản lý tài nguyên trên toàn bộ cụm, ví dụ như hệ thống service mesh (Istio) hay cấp phát chứng chỉ TLS (cert-manager).

## 5. Cơ chế Ủy quyền và Phân quyền (Authorization & RBAC)
